...
export function OrderList({
  ...
  return (
    <section className="rounded-[1.75rem] border border-[var(--line-soft)] bg-[var(--surface-panel)] p-5" id="admin-pedidos">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.3em] text-[var(--ink-soft)]">{copy.eyebrow}</p>
          <h3 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-[var(--ink-strong)]">{copy.title}</h3>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <label className="text-sm font-medium text-[var(--ink-muted)]">
            ...
            <select
              ...
              onChange={(event) => onFulfillmentFilterChange(event.target.value as FulfillmentStatus | "ALL")}
          ...
          return (
            <article key={order.id} className="rounded-[1.4rem] border border-[var(--line-soft)] bg-white/78 p-4">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="font-mono text-xs uppercase tracking-[0.24em] text-[var(--ink-soft)]">{order.id}</p>
                  <h4 className="mt-2 text-lg font-semibold text-[var(--ink-strong)]">{order.contactFullName}</h4>
                  <p className="mt-1 text-sm text-[var(--ink-muted)]">{order.contactEmail} · {order.contactPhone}</p>
                  <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.18em]">
                    <span className="rounded-full border border-[var(--line-soft)] bg-white px-3 py-1 text-[var(--ink-strong)]">{copy.commercial} {order.status}</span>
                    <span className="rounded-full border border-[var(--line-soft)] bg-[var(--surface-panel)] px-3 py-1 text-[var(--brand-deep)]">{copy.fulfillment} {FULFILLMENT_LABELS[order.fulfillmentStatus]}</span>
                    <span className="rounded-full border border-[var(--line-soft)] bg-white px-3 py-1 text-[var(--ink-muted)]">{copy.payment} {paymentStatus}</span>
                  </div>
                  ...
                  <p className="mt-2 text-sm text-[var(--ink-muted)]">CP {order.shippingPostalCode} · {order.shippingPhone}</p>
                </div>
                <div className="flex flex-col items-start gap-3 md:items-end">
                  <strong className="text-lg text-[var(--ink-strong)]">{formatMoney(order.totalAmount, order.currencyCode)}</strong>
                  <button className="rounded-full border border-[var(--line-soft)] px-4 py-2 text-sm font-semibold disabled:opacity-50" disabled={!canCancel || pendingActionKey === `cancel:${order.id}`} onClick={() => void handleCancel(order.id)} type="button">
                    {pendingActionKey === `cancel:${order.id}` ? copy.cancelling : canCancel ? copy.cancel : copy.locked}
                  </button>
                  {!canCancel && order.status === "PAID" ? <p className="max-w-xs text-right text-xs text-[var(--ink-muted)]">{copy.cannotCancelPaid}</p> : null}
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3 rounded-[1rem] border border-[var(--line-soft)] bg-white/70 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--ink-soft)]">{copy.reference}</p>
                  <input className="mt-2 w-full rounded-[1rem] border border-[var(--line-soft)] bg-white px-4 py-3 text-sm text-[var(--ink-strong)]" onChange={(event) => setReferencesByOrderId((current) => ({ ...current, [order.id]: event.target.value }))} placeholder={order.deliveryReference ?? copy.addReference} value={referencesByOrderId[order.id] ?? ""} />
                  <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-[var(--ink-muted)]">
                    <span>{order.paidAt ? `Pago confirmado ${new Date(order.paidAt).toLocaleString()}` : copy.awaitingPayment}</span>
                    {order.deliveryReference ? <span>{copy.currentReference}: {order.deliveryReference}</span> : null}
                  </div>
                  <button className="mt-4 rounded-full bg-[var(--brand-deep)] px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={!canAdvanceFulfillment || pendingActionKey === `fulfillment:${order.id}`} onClick={() => void handleAdvanceFulfillment(order)} type="button">
              ...
              <div className="mt-4 rounded-[1rem] border border-[var(--line-soft)] bg-[var(--surface-panel)] p-4 text-sm text-[var(--ink-muted)]">
                <p className="mb-2 font-semibold text-[var(--ink-strong)]">{copy.orderItems}</p>
